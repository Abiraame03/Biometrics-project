...
# ----------------------------
# Iris and hand feature extraction
# ----------------------------
...
def get_iris_features(img_path, detector, predictor):
...
# =========================================================================
# 🔹 LOAD DATABASES (small files, okay at startup)
face_user_db = {}
...
def authenticate_face(img_path, username, model):
    from deepface import DeepFace
    if username not in face_user_db: return False
    try:
        emb = DeepFace.represent(img_path, model_name="Facenet512", model=model, enforce_detection=True)[0]["embedding"]
    except:
        return False
    best_similarity = -1
    for e in face_user_db[username]:
...
def authenticate_iris(img_path, username, detector, predictor):
    if username not in face_user_db: return False
...
def master_multi_modal_authenticate(username, frame_path):
    ...
    auth_face = authenticate_face(frame_path, username, deepface_model)
    auth_iris = authenticate_iris(frame_path, username, detector, predictor)
    auth_hand = authenticate_hand(frame, username, hands_model)
    matches = sum([auth_face, auth_iris, auth_hand])
    if matches >= 2:
        return True, f"✅ AUTHENTICATION PASSED! ({matches}/3 modalities matched)"
    else:
        return False, f"❌ AUTHENTICATION FAILED ({matches}/3 modalities matched)"
...
if menu == "Enrollment":
    ...
    if st.button("Register User") and username and image_cv is not None:
        ...
        emb_hand = extract_hand_landmarks(image_cv, hands_model)
    ...
    if st.button("Authenticate") and username and image_cv is not None:
        ...
        success, msg = master_multi_modal_authenticate(username, temp_path)
        st.write(msg)
        if success:
